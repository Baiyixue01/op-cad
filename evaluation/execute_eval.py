...
# ========== 计算 3D 指标 ==========
def _safe_get_cd_hd(pred_step_path, gt_step_path):
    """包装 get_cd_hd：异常时返回 MetricsResult(ok=False, reason=...)"""
    from utils.compute_3D import get_cd_hd, MetricsResult  # 引用可用函数
    try:
...
def recompute_full_once(task_tuple: tuple, out_dir: str, gt_single_step_dir: str, op_orient_dir: str, dedup_csv: str):
    ...
    若 k==0 且 single 存在且成功，则直接继承 single 指标。
    否则正常计算（旋转对齐）。
    """
    from utils.compute_3D import MetricsResult
    ...
    if k == 0 and os.path.exists(single_step_path):
        # 读取 single 结果指标
        gt_single_step = resolve_gt_single_path(pid, gt_single_step_dir, dedup_csv)
        if gt_single_step and os.path.exists(single_step_path):
            res_s = _safe_get_cd_hd(pred_step_path=single_step_path, gt_step_path=gt_single_step)
            if res_s.ok:
    ...
    if not inherit_from_single:
        if pred_full_step_exists and gt_full_step:
            # step≥1 或 single 不可用：旋转对齐
            res_f = _safe_get_cd_hd(pred_step_path=full_step_path, gt_step_path=gt_full_step)
        else:
