...
def apply_args(args):
    ...
    global WRITE_SUMMARY, VISUAL_MODE
...
def process_one(r, K, COP,COP_PRE_CODE_DIR, PRE_CODE_DIR, TMP_DIR,
                GT_IMAGE_DIR,GT_SINGLE_STEP_DIR,GT_EDGES_DIR) -> Tuple[List[dict], List[dict], str]:
    ...
    cands = get_model_candidates(prompt, K, image_paths=[visual_image] if visual_image else None)
    ...
    for k_idx, item in enumerate(cands):
        ...
        if op_kind == "chamfer_fillet":
            ...
            cfm = {
        "cf_num_pred_fillet": np.nan,
        "cf_num_pred_chamfer": np.nan,
        "cf_num_gt_fillet": np.nan,
        "cf_num_gt_chamfer": np.nan,
        "cf_hits_fillet": np.nan,
        "cf_hits_chamfer": np.nan,
        "cf_iou": np.nan,
    }
            ...
            row = {
                ...
                "k_index": k_idx,
                "exec_ok_full": np.nan,
                "pred_full_path": np.nan,
                "cd_full": np.nan,
                "hd_full": np.nan,
                "reason_full": np.nan,
                "pred_full_exists": np.nan,
                "metric_ok_full": np.nan,
                **cfm,       # ←← 只包含 7 个 CF 字段
                **gen_meta,
            }
        ...
        row = {
            ...
            "angle_single": np.nan,
            "angle_full": np.nan,
            "reason_single": np.nan,
            "reason_full": np.nan,
            "pred_single_exists":0,
            "pred_full_exists":0,
            "metric_ok_single": np.nan,
            "metric_ok_full": np.nan,
    ...
    summary_row = {
        ...
        "k_index": "summary",
        "op_type": "Geometry",
        "best_k_single": np.nan,
        "cd_single_best":np.nan,
        "hd_single_best":np.nan,
        "cd_single_mean": np.nan,
        "hd_single_mean": np.nan,
        "best_k_full": np.nan,
        "cd_full_best": np.nan,
        "hd_full_best": np.nan,
        "cd_full_mean": np.nan,
        "hd_full_mean": np.nan,
        "n_total": np.nan,
        "n_exec_ok_single": np.nan,
        "n_exec_ok_full":np.nan,
        "n_pred_exist_single": np.nan,
        "n_pred_exist_full": np.nan,
        "n_metric_ok_single": np.nan,
        "n_metric_ok_full": np.nan,
    }
...
# 结果缓冲区 (!!! 提前初始化 !!!)
    ...
    with mp.Pool(processes=NPROC) as pool:
        worker = partial(
            ...
            K=K,
            COP=COP,
            COP_PRE_CODE_DIR=COP_PRE_CODE_DIR,
            PRE_CODE_DIR=PRE_CODE_DIR,
            TMP_DIR=TMP_DIR,
            GT_IMAGE_DIR=GT_IMAGE_DIR,
            GT_SINGLE_STEP_DIR=GT_SINGLE_STEP_DIR,
            GT_EDGES_DIR=GT_EDGES_DIR,
