...
def normalize_sample_id(
    ...
    def _clean_sid(value: str) -> str:
        sid = str(value).strip().replace("/", "_")
        sid = re.sub(r"_?step_?(\d+)$", r"_step\1", sid)
        return re.sub(r"_+", "_", sid)

    if sample_id:
        sid = _clean_sid(str(sample_id))
        if re.search(r"(?:^|_)step\d+$", sid):
            return sid

    gid = _clean_sid(group_index)
    if re.search(r"(?:^|_)step\d+$", gid):
        return gid
    return f"{gid}_step{int(step_id)}"
...
def resolve_highlight_embedding_path(
    ...
    *,
    ...
    1) Column `embedding_path` with an existing file path
    2) `embed_dir / {sample_id}.npy` where sample_id is normalized from group_index (+ optional sample_id column)
    """
    key = "embedding_path"
    raw = None
    if key in row:
    ...
    if embed_dir:
        cand = os.path.join(os.path.abspath(embed_dir), f"{sid}.npy")
